...
| [07-keypad.md](./hardware/07-keypad.md) | 小键盘页面（2026-09-08，09-09 动作注册表重构 + **当晚序列化升级**）：6 键串口输入设备（9600，`<键位>,<on\|off>` **无行尾分隔符**，支持任意键数）；**每键绑定动作序列数组，按下按顺序执行**——模拟按键（koffi 系统级组合键，序列中=完整击键按下按住60ms自动抬起，push-to-talk 已移除；macOS 需辅助功能授权，引用计数+releaseAll 防修饰键卡死）/ 打开应用（本机应用下拉 `appCatalog` 扫描 + creatable 自定义路径）/ 执行脚本（cliRun 任意 shell 命令）/ 权限审批（允许/拒绝最近待审）/ 延时等待（50–60000ms sleep Promise 形成序列间隔）；三端注册表（@common 动作定义 KEYPAD_ACTIONS / main 执行器 KEYPAD_ACTION_EXECUTORS（onPress 可异步、onRelease 已删）/ 渲染层编辑器 KEYPAD_ACTION_EDITORS）映射类型穷尽校验，新增动作=各端一文件+登记一行（文档含三步指南）；序列执行=顺序 await+每键 running 重入守卫、off 仅簿记不分发；**实体键盘外观**：键帽 div+css 立体效果+按下动画（鼠标+设备 pressed 同步）、键帽纵向显示**绑定摘要（binding.name 自定义命名优先，未命名回退首条动作+「共 N 个动作」小字**；app=图标+名称，图标走本地 server `/icon/app` 接口纯 Node 解析 .app 内 icns 提取 PNG 缓存）、点击键帽右侧常驻配置面板（**显示名称输入框（可选）+ KeypadSequenceEditor 序列编辑**：序号+图标+摘要行、上下移/删除、手风琴展开编辑器、添加动作=类型卡片网格、normalize 预校验禁保存、全部删空保存=清除）；键盘样式 `config.layout` 持久化（keypadLayouts 注册表，样式一 4×2 键1竖跨/键6横跨）；存量 `{name,actions}`/纯数组/单动作三代格式归一化零迁移；SerialService `subscribePortData` + keypadProtocol 文法流式解析 |
