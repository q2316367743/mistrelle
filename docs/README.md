...
| [03-article-aside.md](./writing/03-article-aside.md)           | 文章侧边栏（以文档为中心的写作工作台）：一聊天一文档恒可编辑、标题下拉切换 + **标题在 ⓘ 面板内可直接编辑（t-input，失焦/回车提交、IME 组合态不提交、空值不提交回滚；推翻旧「标题仅 AI 可改」拍板）**、类型下拉切换（AI 专属设定）、简介/提纲信息下拉面板（只读）、头部刷新按钮外显、AI 写完自动呈现（article_write→contentRevs + mtime 轮询兜底）、版本时间线（显式 no）、重写入口收敛到聊天（AI 重写按钮已删）、插图/生图直插正文、**插图生图以选中文字为前提（未选中禁用+tooltip 引导，插入不吞选中文字）**、**生图弹窗 AI 代写英文描述（按标题/摘要/提纲 + 选中文字起草，可换一版）**、**封面悬浮框「复制图片」（copyImageByPath 把图片本体写入剪贴板，可直粘）**、底部复制正文 |
...
| [07-keypad.md](./hardware/07-keypad.md) | 小键盘页面（2026-09-08，09-09 动作注册表重构 + 序列化升级，**09-10 打开网页动作 + 短按/长按互斥判定，09-11 媒体键支持 + 长按行为由队列形状推导 + 断开即停**）：6 键串口输入设备（9600，`<键位>,<on\|off>` **无行尾分隔符**，支持任意键数）；**每键绑定动作序列数组，按下按顺序执行**——模拟按键（koffi 系统级组合键：普通键走 CGEventCreateKeyboardEvent / keybd_event，**媒体键走 macOS NX_SYSDEFINED 系统定义事件（type14/subtype8/data1=(NX_KEYTYPE<<16)\|0xa\|0xb<<8，字段号 83/149/150 实测）+ Windows VK_ 系列**，音量/亮度/静音/播放/切曲八种中文名下拉、无修饰语义且不可录制；亮度 Windows 无标准虚拟键静默跳过；**Fn 键归入修饰键（09-12）**（macOS 合成 flagsChanged：kVK_Function 0x3F + kCGEventFlagMaskSecondaryFn 1<<23，setFlags 无条件调用防粘连、主键可缺省=只按住修饰键，用于长按 Fn 触发输入法语音输入，Windows 无 VK 静默跳过）；macOS 需辅助功能授权，引用计数+releaseAll 防修饰键卡死）/ 打开应用（本机应用下拉 `appCatalog` 扫描 + creatable 自定义路径）/ 执行脚本（cliRun 任意 shell 命令）/ 权限审批（允许/拒绝最近待审）/ 延时等待（50–60000ms sleep Promise 形成序列间隔）/ **打开网页**（shell.openExternal 系统默认浏览器，`isValidKeypadUrl` 仅收 http/https）；三端注册表（@common 动作定义 KEYPAD_ACTIONS / main 执行器 KEYPAD_ACTION_EXECUTORS（onPress 可异步）/ 渲染层编辑器 KEYPAD_ACTION_EDITORS）映射类型穷尽校验，新增动作=各端一文件+登记一行（文档含三步指南）；序列执行=顺序 await+每键 running 重入守卫；**短按/长按互斥判定**（`KeypadBinding.holdActions?` 可选长按序列 + 全局阈值 `KEYPAD_HOLD_MS=600`：按下起计时、到时仍按住执行长按 / 阈值内松手执行短按，未配置键位保持按下立即执行零回归，normalizeBinding 清洗空序列不落盘、拔线清 holdTimers、绑定面板短按/长按双区块复用 KeypadSequenceEditor+emptyText prop）；**长按行为由队列形状推导**（无行为配置项，`resolveKeypadHoldBehavior` 两端共用）：单条普通模拟按键=持续按住 keep（pressCombo 不自动抬起、抬手 releaseCombo）；多条=持续循环 repeat（每轮 `holdRepeatMs` 20–5000ms 缺省 100，一轮跑完再等间隔串行不重叠、逐轮重跑整个队列、总时长上限 `KEYPAD_REPEAT_MAX_MS=60000`）；单条非模拟按键=执行一次 once；单条媒体键例外走循环（系统直接消费、保持按住不持续生效）；抬手/保存绑定统一 endHoldSession 结束，渲染层 HoldBehaviorEditor 展示推导行为+仅循环时显示间隔输入；**断开即停**（主动/拔线/换连接/保存绑定统一 resetPressed）：先 endSession 换代中止在途序列（Promise.race 与世代信号竞速，卡在 delay 的 await 立即返回）、清计时/循环/保持会话并 releaseAll，disconnect 先 closePort（同步摘监听）再 reset；`running` 改 Map<keyId,世代> 防旧序列误删新条目；off 仅簿记不分发（配置长按的键位阈值内松手例外）；**实体键盘外观**：键帽 div+css 立体效果+按下动画（鼠标+设备 pressed 同步）、键帽纵向显示**绑定摘要（binding.name 自定义命名优先，未命名回退首条动作+「共 N 个动作」小字**；app=图标+名称，图标走本地 server `/icon/app` 接口纯 Node 解析 .app 内 icns 提取 PNG 缓存）、点击键帽右侧常驻配置面板（**显示名称输入框（可选）+ KeypadSequenceEditor 序列编辑**：序号+图标+摘要行、上下移/删除、手风琴展开编辑器、添加动作=类型卡片网格、normalize 预校验禁保存、全部删空保存=清除）；键盘样式 `config.layout` 持久化（keypadLayouts 注册表：**分组 + 判别 cell + preset 预置**，样式一 4×2 键1竖跨/键6横跨，**09-14 新增样式二 = 左 4×2 八键 + 视觉分隔 + 右 1×2「上旋钮/下按键 9」**）；**旋钮（09-14）**：无极可左右转，**三路各占一个普通键位号**（右转 10/左转 11/按下 12）走普通按键上报 → 协议/动作/执行器/IPC 零改动；**「是否可按压」是布局配置**（`KeypadKnobCell.pressKey` 缺省即不可按压，路切换条与提示均由 `knobRoutes` 从布局派生，不写死三路）；编辑=点旋钮后右侧面板顶部「绑定路」切换条（左转/右转/按下）选中哪路配哪路、复用序列编辑器（面板契约仍单一 keyId）；样式预置仅补未绑定键位不覆盖已有；⚠️设备须 on/off 成对上报（同键位号按下去重，只发 on 会被吞）；存量 `{name,actions,holdActions?,holdRepeatMs?}`/纯数组/单动作多代格式归一化零迁移；SerialService `subscribePortData` + keypadProtocol 文法流式解析 |
