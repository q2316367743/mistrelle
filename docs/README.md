...
| [08-chat-list-project-groups.md](./app/08-chat-list-project-groups.md) | 聊天列表项目分组（2026-09-12）：按聊天绑定的工作目录 `workspace`（创建时锁定）聚合、任务列表置顶+项目分组按最近活跃、分组头可折叠+hover「新建聊天」（`/new?workspace=` 预填，⚠️ 无预填传 undefined 防覆盖手选目录）、useChatGroups composable 拍平 rows 进 VList；09-13 工作空间合并口径：`useWorkspaceList` 单一数据源（聊天绑定 ∪ 目录历史、空分组置底）两侧同源 + 删除（面板条目/分组头右键，确认后连聊天级联删）+ 重命名（拍板仅改显示别名，localStorage `workspace-aliases`，目录路径不动）；09-25 分组内默认只列最近 8 条（`GROUP_VISIBLE_LIMIT`）+ 组内末尾「查看更多（还有 N 条）」/「收起」（展开态不持久化、当前聊天在 8 条外不特殊处理、VList 三分支须显式 `:key`） |
| [09-toolbox-pages.md](./app/09-toolbox-pages.md) | 朝花夕拾工具箱（2026-09-25）：侧边栏分组菜单（生图移入 + 打水印 `/attachment/watermark` + 去 AI 味儿 `/attachment/humanize`，无落地页，两页进 keep-alive）；**打水印页**=把遮盖编辑器抽成 `components/mosaic/MosaicEditor.vue`（props 驱动 + 注入 `MosaicApplier` 出口，画布抽屉只做接线），独立页导出走渲染层离屏 canvas（`drawCoverInto` scale=1，规避 sharp 不修 EXIF）+ `dialog.save` + `writeBinaryFile`；**去 AI 味儿页**=左 Markdown 富文本（`MarkdownRichEditor.vue` 复用文章命令面）+ 右结果（流式、结束后可编辑）、深度弹窗迁 `components/humanize/` |
...
| [10-ai-workspace.md](./chat/10-ai-workspace.md)                     | AiWorkspace 工作目录选择器：单形态 popup 面板（搜索 + 最近目录 + 删除 / 清除 / 替换）、与左侧分组共用 `useWorkspaceList`；位于输入区上方，显隐由 `showWorkspace` 控制（只读锁定形态已于 2026-09-22 删除） |
...
| [22-chat-sender-toolbar.md](./chat/22-chat-sender-toolbar.md) | **发送栏工具条重排与权限模式四档（2026-09-22）**：参考 zcode「输入区 + 工具条」两段式（左 + / 权限模式下拉 / 上下文标签，右 用量 / 模型 / 圆形发送·停止；**工作空间行已于同日上移至输入框上方**，见 23）；模式从附件面板提为工具条一等入口、**新增 `AiChatMode=3` 自动编辑**（文件写入免审批、shell 仍审批、黑名单照旧，无 schema 迁移）；**模式改为随对话实时生效**（`v-model:mode` → `ChatSession.setMode` → AgentChat 代入下一个工具批次 + 立即落盘，`ChatSenderInitial.mode` 移除）；拆出 `useChatSenderEditor` / `LChatModeSelect` / `chatModeOptions` / `LChatSenderTags` / `LChatTokenUsage`，附件面板「模式」收敛为「隐私」 |
| [23-chat-workspace-row.md](./chat/23-chat-workspace-row.md) | **工作空间行上移至输入框上方（2026-09-22）**：发送栏改「工作空间行 / 输入区 / 工具条」三段式；**聊天页整块不渲染**，显隐统一由 `showWorkspace` 一个开关表达（删除 `lockWorkspace`）；连带清理 AiWorkspace 只读形态（readonly prop + `--readonly` 样式 + openWorkspace）与工具条的竖分隔线 |
...
| [09-gzh-scene.md](./writing/09-gzh-scene.md)                   | 公众号场景（2026-09-21，同日补齐 skill 至 8 个）：writing 家族第三个子场景 `gzh`（数据复用 articleStore 同库）；**首个使用 `SceneDefinition.skills` 的场景**（8 个 gzh-* 内置 skill=长文/短文/定位三件套/标题/爆款数据/排版/封面/配图，?raw 打包 + skillCatalog 目录 + load_skill 兜底；gzh-trends 含默认赛道组 + 多赛道对比）；排版本地渲染器（marked + 元素 style 映射，AI 不生成 HTML）+ 侧边栏 GzhAside（复用 article 组件 + 底部动作条「更多」分组 + 全覆盖浮层能力面板：排版预览/正文质检，v-show 保留质检结果）+ `gzh_trends` 爆款数据工具（Python 脚本 TS 重写迁 main，gzh:trends IPC 四件套，⚠️ 源站需 no-SNI TLS 回退）；风格库 `~/.mistrelle/gzh-style/` + 4 内置预设 + internal 风格工具（toolMap 接线不建组）；**二期扩展位**：global-content-search（agent-reach/bili-cli/Guaikei）仍待做；xhs-hotnotes 已于 2026-09-26 随小红书场景落地（见 writing/10） |
| [10-xhs-scene.md](./writing/10-xhs-scene.md)                   | 小红书场景（2026-09-26）：writing 家族第四子场景 `xhs`（文复用 articleStore 同库、type「小红书」；图走**画布画板**——一页一个画布文档、1080×1440、底部 18% 遮挡区，刻意不用 HTML 引擎；取数走红狐）；10 个 `xhs-*` 内置 skill（?raw 打包，源包 xhs-Skills 按本项目工具面整体改写：HTML→画布节点、`image_gen`→`image_generate`、WebSearch→`any_search`、脚本→`image_info`/`file_read_xlsx`/`canvas_inspect`）；侧栏 = `t-tabs` 两个同位视图「图片 / 正文」（**图为主**：图片 tab 常驻画板、正文 tab 用 Monaco 编辑源码纯文本且**不做 Markdown 渲染**，均 `destroyOnHide:false`、正文 lazy 首次可见才挂 Monaco）；浮层外壳 `PanelOverlay` 上移为写作家族共享（gzh 同步改用）；**红狐取数工具 `xhs_hot_notes`** 走 main `appAxios`（`xhs:hot-notes` IPC 四件套 + 域闸 + 未配置降级公开检索），凭证=第三方账号新增 `redfox` 字段（经 IPC 下发、main 不留存）；场景提示词工厂 `buildXhsScenePrompt`（生图 / 取数段落与工具注入同源门控），不拼 `buildArticleScenePrompt`（其 16:9/1:1 配图规格与 3:4 画布冲突） |
