...
| [07-keypad.md](./hardware/07-keypad.md) | 小键盘页面（2026-09-08，09-09 动作注册表重构 + 实体键盘外观）：6 键串口输入设备（9600，`<键位>,<on\|off>` **无行尾分隔符**，支持任意键数）；**每键绑定一种动作（注册表驱动，可持续扩展）**——模拟按键（koffi 系统级组合键，macOS 需辅助功能授权，引用计数+releaseAll 防修饰键卡死）/ 打开应用（本机应用下拉 `appCatalog` 扫描 + creatable 自定义路径）/ 执行脚本（cliRun 任意 shell 命令）；三端注册表（@common 动作定义 KEYPAD_ACTIONS / main 执行器 KEYPAD_ACTION_EXECUTORS / 渲染层编辑器 KEYPAD_ACTION_EDITORS）映射类型穷尽校验，新增动作=各端一文件+登记一行（文档含三步指南）；**实体键盘外观**：键帽 div+css 立体效果+按下动画（鼠标+设备 pressed 同步）、点击键帽右侧常驻配置面板（右上角 X 关闭，替代原 popup）、键盘样式 `config.layout` 持久化（keypadLayouts 注册表，样式一 4×2 键1竖跨/键6横跨）、键帽显示动作摘要（app=图标+名称，图标走本地 server `/icon/app` 接口纯 Node 解析 .app 内 icns 提取 PNG 缓存）；面板草稿式编辑（normalize 预校验禁保存）；存量无 type 旧格式归一化回退 combo 零迁移；SerialService `subscribePortData` + keypadProtocol 文法流式解析 |
