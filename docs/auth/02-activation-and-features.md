...
- **AI 面直接过滤/拒绝**：模型不可发现不可用不可写（font_list 过滤、list_design_styles 过滤、create/update 返回明确 error）。
- **渲染不拦**：已有画布/文档中已用到的资源库字体（fontRegistry.ensureFontsForDoc）与已有会话引用的自建风格（get_design_style、ChatSessionManager.getDetail）**继续可读可渲染**，防旧作品损坏与进行中会话 brick；数据保留磁盘，升级后自动恢复全部能力。
