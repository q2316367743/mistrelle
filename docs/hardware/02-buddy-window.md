...
> 伙伴窗口：独立于 AI 主窗口的第二个窗口，独立 HTML 入口（`buddy.html` → `src/renderer/src/windows/buddy/`），承载硬件控制等独立页面。默认隐藏，托盘「打开伙伴」为唯一入口。
...
| renderer | `src/renderer/buddy.html` | 独立 HTML 入口（标题「伙伴」，script 指向 `/src/windows/buddy/main`） |
