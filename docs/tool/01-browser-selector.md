...
- `markdown` 模式：fetch 载荷带 selector 时，main 的 `BrowserToolRunner` 先用页内 `extractHtml(selector)`
  取元素 HTML，再 turndown 转换（`src/main/src/modules/browser/pageScripts.ts`）。
...
- 提取逻辑为 main 侧共享函数 `extractHtml` / `extractText`（`src/main/src/modules/browser/pageScripts.ts`）。
...
- `src/main/src/modules/browser/runner.ts` + `pageScripts.ts`（步骤解释与页内提取，见 `docs/browserTool/01-browser-tool.md`）
