...
- **固定提示词**（`buildDesignCanvasPrompt({ hasImageGenerate })` 动态组装）：角色（资深平面设计师）、工作流（选比例 → 定 palette → batch
  ...
  **布局驱动铁律**（区域内排版的根因清单：节点属性必须写进 `node` 内部、卡片高度优先 `hug_contents`、padding 用四元素 `[上,右,下,左]`、
...
- **区域排版规范（防贴边 / 防无间距 / 防边距不等）**：`gap` / `layout` / `padding` / `height` 等全部是**节点属性**，必须写在
  `node: {...}` 内部（操作级只有 `op` / `parent` / `as`，写错层级被静默忽略或报错）；卡片高度优先 `hug_contents`（内容自适应 + 对称 padding →
  上下边距天然一致），固定 height 会内容堆顶、底部留白失衡；`padding` 两元素数组 = `[垂直, 水平]`，多边距一律用四元素
  `[上, 右, 下, 左]` 消除歧义；vertical 容器行间距由 `gap` 显式控制，忘写 = 行全挤在一起；多张同构卡片先建一张含 gap / padding 的
  完美模板再 `copy` 复用；跨批引用 `parent` 用上一批返回的真实 id（`@绑定名` 仅同批有效）。
