...
- **chatStream**：`POST {server}/v1/chat/completions`（Bearer apiKey + 透传 `session_id` / `request_id`），服务端按积分记账、代理到 new-api 单上游；流式 SSE 字节经 `onChunk` 回调回传，协议解析仍在渲染层（复用 chat 适配器）。
...
- `AiRequestParams` 增加 `builtin?: boolean`、`sessionId?: string`、`requestId?: string`。
