...
- **record_memory 收尾锚定**：AI 常在回答结束后追加 `record_memory` 调用及一小段补充说明，此时最后一条文本并非真正回答。`finalContent` 判定规则：
  1. `isRecordMemoryCall`（`type === 'toolcall' && data.toolCallName === 'record_memory'`，硬编码风格与 RChatTool 一致）找最后一个 record_memory 的索引。
  2. 其后仍存在其他 toolcall / thinking（`hasWorkAfterMemory`）→ 记忆后还在继续执行，非收尾调用，维持默认逻辑，避免「中途记忆后继续干活」场景回退。
  3. 否则视为收尾调用：最终回复锚定到它**之前**的最后一条文本（`findLastText` 倒序查找）；其前无文本（整轮只有记忆）则回退默认逻辑取其后文本，避免误显「异常停止」。
...
- 完成且折叠：仅保留 `finalContent` + 全部 `continueHint`（继续按钮是操作入口，必须始终可见）；收尾的 record_memory 调用与其后的补充说明归入过程，折叠态不显示（仍计入 `processCount` 步数）。
- 完成且展开 / 无过程可折叠：全量。
