...
```text
┌─ 工作空间行（仅新建页，聊天页整块不渲染，2026-09-22 上移至此）──┐
┌─ 输入区（占位文案 / tipTap 编辑器）─────────────────────────┐
├─ 工具条 ──────────────────────────────────────────────────┤
│ [+] [权限模式 ⌄] [上下文标签…]        [用量] [模型] [●↑]      │
└──────────────────────────────────────────────────────────┘
...
- 左组：`+` 附件面板 → 权限模式下拉 → 上下文标签（Agent / 设计风格 / 隐私）。
- 工作空间选择器见 [10-ai-workspace.md](./10-ai-workspace.md)，**已从工具条移到输入区上方**，详见 [23-chat-workspace-row.md](./23-chat-workspace-row.md)。
...
- **工作空间已不在工具条**：`lockWorkspace` prop 已删除，显隐统一由 `showWorkspace` 表达（聊天页传 `false`，整块不渲染），详见 [23-chat-workspace-row.md](./23-chat-workspace-row.md)。
