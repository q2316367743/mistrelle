...
> 依赖：`LinkPreviewDrawer`（同目录，处理 http 链接预览）；主进程 `src/main/src/app/protocol.ts`（mistrelle:// MIME 表）
...
2. **MIME 表已补**：`src/main/src/app/protocol.ts` 的 `MIME_BY_EXT` 原只覆盖字体 / 图片，`.html` 回退 `application/octet-stream` 无法渲染；现补充 `.html/.htm → text/html`、`.js/.mjs → text/javascript`、`.css → text/css`、`.json → application/json`，本地页面及其相对资源均可正常加载。
