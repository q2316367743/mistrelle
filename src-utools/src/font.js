...
const sanitizeMeta = (meta) => {
  ...
  for (const k of META_KEYS) {
    const v = meta[k]
    if (typeof v === 'string' && v && v !== '全部') out[k] = v
  }
...
const writeFontCache = async (fonts) => {
...
const addFont = async (srcPath, meta = null) => {
  ...
  return withIndexLock(async () => {
    ...
    const out = { name, path: target, source: 'library' }
 ...
 */
const updateFontMeta = async (name, meta) => {
  const cleanMeta = sanitizeMeta(meta)
...
module.exports = {
  ...
  async listFonts() {
    ...
    for (const f of system) merged.set(f.name, { name: f.name, path: f.path, source: 'system', ...(f.meta ? { meta: f.meta } : {}) })
