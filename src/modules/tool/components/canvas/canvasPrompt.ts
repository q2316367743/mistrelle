...
export const DESIGN_CANVAS_PROMPT = [
  ...
  '## 画布排版防错规则',
  '',
  '**规则一：文字占位按「字号 × 1.2~1.4」估算，间距留 8px 以上**',
  '- 错误案例：`28.13`（字号64，y=896）与下方 `环比 ▼ 14.76%`（y=972）重叠。我当时按 `y + 字号`（960）判断安全，但行高 AUTO≈1.2×字号，实际文字盒底部到约 972，正好撞上下一行。',
  '- 正确做法：两个相邻文字的垂直间距 = `y₂ − y₁`，必须 ≥ `字号₁×1.2` + 8px 余量。**y + fontSize 是低估，不可用作底部边界**。',
  '',
  '**规则二：圆点/徽章等元素用「几何中心」定位，不要目测坐标**',
  '- 错误案例：40×40 的排名圆点，行盒 y=1124、高100（中心=1174），我随手放 y=1142，圆点中心只有 1162，偏上 12px（约一个半径）。',
  '- 正确做法：先算参照中心，再反推坐标——圆点 `y = 容器中心Y − 直径/2`；圆内文字 `y = 容器中心Y − 行高/2`。所有"看起来要居中"的元素一律先算中心，禁止凭感觉填 y。',
  '',
  '**规则三：文字宽度先估算，容器宽度 ≥ 内容宽度 + 边距**',
  '- 错误案例：`WEEKLY RANKING · 全球周榜`（约280px）塞进 260px 胶囊，触发换行。',
  '- 估算公式：中文 ≈ 字数×字号；拉丁字母/数字 ≈ 字符数×0.55×字号；含空格与符号再加 10~20px。反过来做更稳：**先写文案 → 算出宽度 → 再定容器宽度**（或先定宽度 → 砍文案），宽容器 + 短文案永远比窄容器 + 长文案安全。',
  '',
