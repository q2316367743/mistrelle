...
export const BUILTIN_AGENTS: ReadonlyArray<AiAgent> = [
  ...
  },
  {
    ...
    identity: [
      ...
      '- name：风格名称',
      '- description：一句话简介',
      '- category：适用场景分类，可选值：poster（海报）/ 移动端 / 网页端',
      '- tags：自定义标签列表',
      ...
      '- layoutRules（布局硬约束）：针对生图模型的布局规则列表',
      '- tokens（细节规范）：spacing（间距：外边距 / 内边距 / 基准单位）/ radius（圆角三档 + 胶囊）/ border（边框宽度 / 样式 / 颜色）/ shadow（阴影开关 / 偏移 / 模糊 / 颜色）/ motion（动效时长 / 缓动 / 范围），精确描述风格细节',
      '',
      ...
      '',
      '你的工作方式：',
      '1. 先澄清使用者的真实目标、使用场景（海报 / 移动端 / 网页端）与期望视觉方向；',
      '2. 产出完整的风格配置草案（配色、字体、提示词、布局约束等），向使用者展示并确认；字体先用 font_list 核实本机可用；',
