...
// ~/.mistrelle/project/{id}/notes
export const buildProjectNoteDirPath = (id: string) =>
  window.preload.path.join(buildProjectDirPath(id), 'notes')
...
/**
 * 新建项目时建立子目录骨架：files / chat / tasks / notes 与 dynamics.json
 */
export const projectCreateSkeleton = async (id: string) => {
  ...
  await Promise.all([
    ...
    window.preload.fs.mkdir(window.preload.path.join(dir, 'notes'))
  ])
