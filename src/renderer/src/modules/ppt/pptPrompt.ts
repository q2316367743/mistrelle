...
export const buildPptPrompt = (): string =>
  [
    ...
    '- 查看：ppt_info 拿文档级信息（页数 / theme），ppt_get_nodes 拿指定页完整元素树（含每个节点 id）；核对元素渲染后的精确位置 / 尺寸 / 间距用 ppt_inspect（画布绝对包围盒，flex 布局后的真实几何以它为准，不要拿 attr 推算）',
    '- 编辑：ppt_batch_edit(pptId, slideId, operations) 对指定页做批量操作（insert / copy / update / move / delete，≤15 个/批）：insert 插入新元素、update 按 id 改样式 / 文本、move 重排、delete 删除。构建层级**优先在一个 insert 的 node.child 递归嵌套整棵子树**（文字内容 = 节点顶层 child 字符串）；仅当后续 op 需引用刚插入的节点时才用 as 绑定名 + parent:"@绑定名" 分步插入',
    ...
    '设计铁律：',
    '- 画布 16:9（1280×720），页面**唯一**根容器用 attr.w="100%" attr.h="100%"（或 max）铺满；正文 / 装饰全部挂在该容器下——**一个 insert 的 node.child 嵌套整页内容**（跨批先 ppt_get_nodes 拿根 id 再 parent:"<id>" 续插）。禁止多个满高根或根外裸 Text——会纵向叠放超出画布',
    '- **每页根元素必须是 VStack / HStack 布局容器**（flexbox 先布局后内容）：垂直分组 VStack、水平分组 HStack、层层嵌套，禁止散落裸 Text / Shape',
    '- 大装饰 Shape（色块 / 圆）若不参与文档流，必须 position:"absolute" + top/left，否则会把标题顶出画布',
    ...
    '错误自纠：编辑或渲染失败会返回完整错误文本（操作校验 / 页码越界 / 渲染错误 / potentialIssues 越界提示等），先分析原因（常见：忘了 as 绑定、向 root 重复插满高容器、insert 的 node 非法、child 写错位置——子树只能嵌在 node.child 数组、文字只能是节点顶层 child 字符串、根元素不是布局容器、页码越界、节点 id 失效），修正后重试，不要盲目重复。'
  ].join('\n')
