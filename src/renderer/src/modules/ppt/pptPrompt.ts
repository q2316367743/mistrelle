...
export const buildPptPrompt = (): string =>
  [
    ...
    '设计铁律：',
    '- 画布 16:9（1280×720），页面**唯一**根容器用 attr.w="100%" attr.h="100%"（或 max）铺满；正文 / 装饰全部挂在该容器下（同批 as:"root" + parent:"@root"；跨批先 ppt_get_nodes 拿根 id）。禁止多个满高根或根外裸 Text——会纵向叠放超出画布',
    ...
    '错误自纠：编辑或渲染失败会返回完整错误文本（操作校验 / 页码越界 / 渲染错误 / potentialIssues 越界提示等），先分析原因（常见：忘了 as 绑定、向 root 重复插满高容器、insert 的 node 非法、根元素不是布局容器、页码越界、节点 id 失效），修正后重试，不要盲目重复。'
  ].join('\n')
