# 布局系统经验（来自官方 layout-system 文档转写）
...
- `position="absolute"`（`"position": "absolute"`）+ `top/right/bottom/left`：脱离文档流，不影响兄弟布局
...
5. **Sidebar（侧栏布局）**：HStack 内左侧 VStack 设 `"w": "25%"` `"h": "max"` 做深色导航栏（`backgroundColor` 深色 + 浅色文字），
   右侧 VStack 做主要内容区。

## 7. 布局最佳实践

- **每页唯一满高根**：页根隐式纵向排列；只放一个 `VStack`/`HStack`（`w/h: 100%` 或 `max`），其余内容全部进该容器的 `child`。多个满高根或「满高根 + 根外兄弟」会超出 720 画布（预览 `overflow:hidden` 表现为内容被裁切）。
...
背景：文本宽度由紧凑字体 **Noto Sans JP** 测算，框宽 = 测宽 + 10px 缓冲；但打开方（WPS / macOS 快速预览）会替换更宽的字体（如微软雅黑）。**拉丁字母 / 数字 / 空格占比高的行在替换字体下更容易超宽**，一旦超宽会按框重新换行，多出的一行就溢出顶到下方元素。

写作经验（按有效性排序）：

1. **单行文本（标题 / 徽章 / 标签）宁短勿长**：避免无空格长英文连串（超长单词、全大写长句、长 URL）；必要英文尽量短；中文一行 1em/字最稳，中文字体间宽度差异小。
2. **中英混排按「英文更宽」预留**：英文 + 数字 + 空格越多，越要缩短文本或拆短句，不要让它单行硬撑到接近容器边缘。
3. **长内容拆行**：需要多行时用 `\n` 拆成独立段落（POM 按行布局，每行各自适配），比「一个超宽单行」安全。
4. **关键文本给足宽度**：对必须存在的单行文本（如卡片标题、表格列名），可在 Text 上显式 `w`（像素）留出比内容更宽的余量，或在容器上留足 padding，避免文字贴边。
5. 字号越大风险越高：大字号（>28）下的英文长句最容易超宽，优先缩短文本或换行。
