# 样式经验（来自官方 styling-guide 文档转写）

## 1. 颜色系统

- **颜色一律 6 位 hex 不带 #**（如 `F8F9FA`），全篇统一格式。
- 创建时用 theme 声明全局色板：`{ "surface": "0F172A", "accent": "38BDF8", "textMain": "F8FAFC", "textMuted": "94A3B8" }`，
  之后 `"backgroundColor": "$surface"`、`"color": "$accent"` 引用； **换主题只改一处**。
- 推荐色板（角色 → 色值）：
...
- KPI 大数字 + 小单位：大数字 Text（`"fontSize": "48"` `"bold": "true"`）+ 小单位 Text（`"fontSize": "20"`）放 HStack
  （`alignItems="end"`）同基线组合。
- 文本效果：`highlight`（高亮底）、`underline`（`true` 或 `underline.style="wavy"` 波浪下划线）、`glow`（发光）/ `outline`
  （描边）原生导出可编辑（glow 需 PowerPoint 查看，LibreOffice 不渲染）。
...
- **卡片标准样式**（信息块通用）：
  ...
  ```
- **侧边强调条**：`"border.color": "E5E7EB"` `"border.width": "1"` + `"borderLeft.color": "1D4ED8"` `"borderLeft.width": "6"`
  （细框 + 粗强调线）；标题下划线用 `borderBottom`。
...
- **卡片内层级**：标签 `"fontSize": "12"` `"color": "999999"` + 数值 `"fontSize": "28"` `"bold": "true"`（涨跌用语义色：
  绿 `16A34A` / 蓝 `1D4ED8`）。
