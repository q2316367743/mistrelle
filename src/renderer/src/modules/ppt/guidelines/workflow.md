...
3. **ppt_batch_edit (pptId?, slideId, elements)**：编辑指定页内容（SlideNode JSON 数组，整页覆盖）。先规划页面结构（封面 / 目录 /
   内容页 / 结尾），再逐页填充。
4. **ppt_read (pptId?, slideId?)**：查看当前 JSON（给 slideId 只看单页元素数组）；渲染报错先读错误修正。JSON 里每个节点带顶层 `id`。
5. **ppt_edit_element (pptId?, slideId, nodeId, patch)**：用户点选节点并引用后，按 nodeId **只改该节点**（改单个文案 / 样式）；
   找不到 nodeId 先 ppt_read 重读。改动大（重构整页结构）才用 ppt_batch_edit 整页重建。
6. **ppt_select (page)**：定位侧边栏预览视角，与用户浏览同步。
7. 需要时 **ppt_export_pptx / ppt_export_png** 导出给用户确认；用户要求新版本时再 **ppt_create** 新文件。
8. 语法 / 布局 / 样式不确定时先读指南：layout（布局模式）/ nodes（节点属性）/ styling（配色字体样式）/ json（存储结构速查）。
