    ...
    <div class="project-detail-layout">
      <nav v-if="project" class="project-detail-tabs">
        <router-link
          ...
          :class="[
            'project-detail-tab',
            { 'is-active': t.active ? t.active($route.path) : t.path === $route.path }
          ]"
        >
          {{ t.label }}
        </router-link>
...
const showFooter = computed(
  () => ['dynamics', 'task'].map((e) => `/project/${id.value}/${e}`).includes(route.fullPath)
)
...
const tabs = computed(() => [
  ...
  { key: 'subscribe', label: '订阅', path: `/project/${id.value}/subscribe` },
  { key: 'note', label: '笔记', path: `/project/${id.value}/note` }
])
