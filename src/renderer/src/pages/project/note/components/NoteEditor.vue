<template>
  <div class="note-editor">
    <editor-content :editor="editor" class="note-editor__content" />
  </div>
</template>
...
import { NoteSlash } from './NoteSlash'
...
const props = defineProps<{
  ...
  /** md 所在目录（图片相对路径解析基准） */
  baseDir: string
  /** 当前笔记 key（相对 note 根，不含 .md；图片落盘到 {name}.assets/） */
  noteKey: string
}>()
...
/** 粘贴 / 拖入本地图片：写入 {name}.assets/ 并插入相对路径节点 */
const insertLocalImage = async (file: File) => {
  try {
    const rel = await saveNoteImage(props.root, props.noteKey, { file })
    editor.value
      ?.chain()
      .focus()
...
const editor = useEditor({
  extensions: [
    ...
    Markdown,
    NoteImage.configure({ baseDir: props.baseDir }),
    NoteSlash.configure({ root: props.root, noteKey: props.noteKey }),
    TableKit
  ],
  content: props.content,
  contentType: 'markdown',
  editorProps: {
    attributes: { class: 'note-editor__pm' },
...
/** 外部内容变化（切换笔记由父级 :key 重挂载，此处兜底外部写入）时仅在确有差异时同步 */
watch(
  () => props.content,
  (value) => {
