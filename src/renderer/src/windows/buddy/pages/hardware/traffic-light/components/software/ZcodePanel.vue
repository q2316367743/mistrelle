<template>
  <div class="zcode-panel">
    ...
    <div v-for="group in ZCODE_GROUPS" :key="group.title" class="binding-group">
...
import { ZCODE_HOOK_EVENTS } from '@/windows/buddy/pages/settings/integrations/registry'

defineOptions({ name: 'ZcodePanel' })

/** 事件分组过滤到 ZCode hooks 能上报的子集（其余事件不投递，不提供绑定） */
const ZCODE_GROUPS = BUDDY_EVENT_GROUPS.map((group) => ({
  title: group.title,
  events: group.events.filter((event) => ZCODE_HOOK_EVENTS.includes(event))
