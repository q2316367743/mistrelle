...
import { LCD_BAUD_RATES, LCD_DEFAULT_BAUD_RATE } from '@common/types/esp32Lcd'
import { useEsp32Lcd } from '../useEsp32Lcd'

defineOptions({ name: 'LcdSerialPanel' })

const { config, connectedPath, connect, disconnect } = useEsp32Lcd()

const ports = ref<SerialPortItem[]>([])
const selectedPath = ref('')
const selectedBaud = ref<number>(LCD_DEFAULT_BAUD_RATE)
const connecting = ref(false)
const listing = ref(false)

const baudOptions = LCD_BAUD_RATES.map((rate) => ({ label: String(rate), value: rate }))
...
function toggleConnection(): void {
  ...
  if (selectedPath.value) {
    void connect(selectedPath.value, selectedBaud.value).finally(done)
    return
  }
  done()
}
...
watch(
  config,
  (next) => {
    if (next) selectedBaud.value = next.baudRate
  },
  { immediate: true }
)
