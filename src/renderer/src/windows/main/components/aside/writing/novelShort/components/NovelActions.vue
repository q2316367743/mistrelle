<template>
  <div class="novel-actions">
    ...
    <t-tooltip content="在文件夹中显示当前小说">
      <t-button size="small" variant="text" :disabled="humanizing" @click="emit('reveal')">
        <template #icon><folder-open-icon /></template>
        文件夹
      </t-button>
    </t-tooltip>
    <t-tooltip content="复制当前文件 Markdown 到剪贴板">
      <t-button size="small" variant="text" :disabled="humanizing" @click="emit('copy')">
        <template #icon><copy-icon /></template>
        复制
      </t-button>
    </t-tooltip>
    <div class="novel-actions__spacer" />
    <span class="novel-actions__words">{{ words }} 字</span>
  </div>
</template>
