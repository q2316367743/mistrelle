...
/**
 * 取批量操作的目标节点 id（锚点白名单校验用）：
 * - update：path 第一段（'父id;子id' 取父）；以 @绑定名 开头 = 同批 insert 的新节点，视为放行
 * - copy / move / delete / image：目标 id
 * - insert：无目标，放行
 */
const batchOpTargetId = (op: CanvasBatchOp): string | null => {
  switch (op.op) {
    case 'update': {
...
/** 锚点白名单校验：写操作的目标必须属于锚点集合，否则拒绝整个批次（读全局、改局部） */
const checkAnchorWhitelist = (ops: CanvasBatchOp[], anchorIds: string[]): string | null => {
  const set = new Set(anchorIds)
  for (const op of ops) {
    const target = batchOpTargetId(op)
    if (target && !set.has(target)) {
      return `锚点修改模式：操作对象「${target}」不在用户选中的锚点元素内（${anchorIds.join('、')}），已拒绝整个批次。只允许修改选中的锚点元素；如需新增元素请用 insert。若用户需求涉及其他元素，请先让用户重新选择。`
    }
  }
  return null
}
...
export const createCanvasTools = (ctx: CanvasToolContext): ToolFunction[] => {
  ...
  return [
    ...
    },
    {
      ...
      label: '批量编辑画布',
      description:
        '核心编辑工具：一次批量执行多个图层操作（insert / copy / update / move / delete / image），≤15 个/批。参数经严格校验：单个操作非法只让该操作失败（错误写入返回的 results），其余操作正常执行；构建顺序建议 背景→主视觉→装饰→文字。详见 canvas_guidelines("operations")',
      parameters: {
        type: 'object',
        properties: {
          operations: {
      ...
      handler: async (...params: unknown[]) => {
        ...
        const anchorIds = ctx.getAnchorNodeIds?.() ?? []
        if (anchorIds.length) {
          const rejected = checkAnchorWhitelist(operations, anchorIds)
    ...
    },
    {
      ...
      handler: async (...params: unknown[]) => {
        ...
        if (anchorIds.length) {
          return {
            error:
              '锚点修改模式：不允许修改调色板（会影响所有引用 $token 的元素）。如需调整整体色调，请取消选中元素后发起全局修改。'
          }
        }
    ...
    },
    {
      ...
      label: '获取设计参考',
      description:
        '获取内置设计参考（按需加载，避免全部塞进提示词）。通用：style-guide 反 AI 俗套 / composition 构图 / typography 字体排版 / operations 批量编辑与节点速查 / workflow 端到端工作流 / image-generation 生图省钱 / styles 内置风格目录（签名手法）；场景：poster / book-cover / album-cover / social-media / knowledge-card。做某类作品前先读对应场景指南；未指定风格时先读 styles',
      parameters: {
        type: 'object',
        properties: {
          topic: {
            type: 'string',
            description: `可用 topic：${CANVAS_GUIDELINE_TOPICS.join(' / ')}`
          }
        },
        required: ['topic']
      },
