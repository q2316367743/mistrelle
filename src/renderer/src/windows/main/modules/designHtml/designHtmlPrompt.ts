 ...
 * 类型创建后内容稳定，仅随运行时设置动态组装：配置了默认生图模型（image_generate 已注入）
...
const IMAGE_GENERATE_RULES = [
  ...
  '- 无真实素材的插画 / 人物 / 场景 / 纹理 / 抽象视觉 → **用 image_generate(prompt, path?) 生成**（已配置默认生图模型，工具可用），把返回的本地 path 填进 `<img src>`；生成失败或服务不可用时才回退几何图形组合。',
