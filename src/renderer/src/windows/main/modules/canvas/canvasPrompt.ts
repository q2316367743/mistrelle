...
const DESIGN_CANVAS_BEFORE_VISUAL = [
  ...
  '',
  '### 工作流程',
  '1. 先问投放平台与用途再 canvas_create（按用途选比例：海报 3:4 1080×1440、电影海报 2:3、专辑封面 1:1、公众号封面 2.35:1 900×383、小红书 3:4、知识卡片 4:3）；文案先压到场景额度再排版；一次只专注一个画布',
  '2. 风格：已锁定 designStyleId 则落地其签名手法；未指定则先 canvas_guidelines("styles") 报 3~4 个候选，禁止只换色板；再 canvas_set_palette 定义 3-5 个颜色 token，fill/stroke 用 $token名',
  '3. 用 canvas_batch_edit 构建（每批 ≤ 10 个操作，参数严格校验，单个操作非法仅该操作失败、其余照常）：先按「区域分组铁律」拆出区域并建 group 骨架（内联 children 或 as 绑定名一批搭好），再往 group 里填子元素；顺序 背景 → 主视觉 → 装饰 → 文字',
...
const DESIGN_CANVAS_AFTER_VISUAL = [
  ...
  '- **vertical 卡片行间距靠 `gap` 显式写**：vertical 容器内标题 / 数据 / 正文的行间距由 gap 控制，忘写 = 全部挤在一起。写 `gap: 18~24`（卡片内行距 ≥ 24 防贴死），并配套正文 `lineHeight: 1.5` + 灰色正文形成呼吸感',
  ...
  '',
  '### 按需加载详细规则',
  '- canvas_guidelines("styles")：内置风格目录（id / 别名 / 签名手法 / 画幅）；未指定风格时先读并协商',
  '- canvas_guidelines("style-guide")：反 AI 俗套铁律 + 风格协商 / 四步法',
  '- canvas_guidelines("composition")：构图法则 + 定量门槛 + 常用画布尺寸',
  '- canvas_guidelines("typography")：字体排版（层级/中文行长/字距）',
  ...
  '- 做任何设计前，至少先读 composition 与 typography；做具体类型作品前先读对应场景指南；开工写 batch_edit 前先读 operations；风格细节以锁定的设计风格或 styles 目录为准，禁止只换色板',
  ...
  '',
  '**规则三：先写文案 → 估算宽度 → 再定容器宽度 → inspect 验证**',
  '- 容器宽度 ≥ 内容宽度 + 边距，否则换行。估算（中英混排分段累加）：`宽 ≈ 中文字数×字号 + 拉丁字符数×0.55×字号`；含空格与符号再加 10~20px。',
  '- **估算后立即用 canvas_inspect 验证文字实际宽 ≤ 内容区宽（内容区 = 容器宽 − 2×padding）**，不等用户发现；中英混排最容易低估（如估 240 实 430）。宽容器 + 短文案永远比窄容器 + 长文案安全。',
  '',
  ...
  '',
  '**规则八：收尾全局收口检查（先量后摆的最后一步）**',
  '- 每张卡 / 每块区域收尾前，用 canvas_inspect 全局核对底部元素：底边距画布底边 ≥ 顶部留白（参考 1180 安全线经验），且四边 ≥ 画布短边 × 4%（1080×1440 → ≥48px）。',
  '- **改一处查三处**：任何一次改动后，同时核对上方间隙（是否贴死 / 重叠）、下方重叠（是否压到相邻元素）、左右对齐（是否与同类元素同线）——禁止只查当前改动处。',
  '',
  '**通用原则**：所有元素排布都用"先量后摆"——先定网格与间距（间距一次给足，卡片内 gap ≥ 24、padding ≥ 36-44、区块间距 ≥ 32-40，不要"先紧后松"），再估宽、再用数学反推坐标（参照物：行中心 / 容器边界 / 文字盒），文案宁短勿长。'
]
