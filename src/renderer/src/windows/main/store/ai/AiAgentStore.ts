...
/**
 * 绑定会员能力的内置 Agent（id → 所需 feature key）：
 * - builtin:design-style 创建 / 修改自定义设计风格，依赖 features.extendedDesignStyles
 * - builtin:card-style 创建 / 修改自定义卡片风格，依赖 features.extendedCardStyles
 * 非会员隐藏对应内置 Agent（不可选、不可见；会员期内用它开过的历史会话经 getById 仍可继续，见 getById 注释）。
 */
const BUILTIN_AGENT_FEATURE_GATES: Readonly<Record<string, AuthFeatureKey>> = {
  'builtin:design-style': 'extendedDesignStyles',
  'builtin:card-style': 'extendedCardStyles'
}
...
export const useAiAgentStore = defineStore('ai-agent', () => {
  ...
  const all = computed<Array<AiAgent>>(() => {
    const features = useAuthStore().features
    const builtins = BUILTIN_AGENTS.filter((a) => {
      const key = BUILTIN_AGENT_FEATURE_GATES[a.id]
  ...
  const getById = (id?: string): AiAgent | undefined => {
    ...
    // 不在 all 中查找：all 会按会员档过滤会员专属内置 Agent（设计风格 / 卡片风格创建助手），
    // 而会员期内用这些 agent 开过的历史聊天（agentId 持久化）免费档仍需可继续 →
    // 直接在全量底层查找（同 DesignStyleStore.getDetail 保留渲染先例）。
    // 仅「新建 / 切换」入口经 all 不可见，达到隐藏语义。
    return (
